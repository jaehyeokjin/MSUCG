...
# Initial variables
outputname = "mscgfm_input.lammpstrj"
inputname = "cg.lammpstrj"
f = open(inputname, 'r')
out_data = open(outputname, 'w')
k = 0
count_time = 0
index = [i for i in range(0, 1050)]  # (# of CG sites) = 1050 in this case
index_temp = []
coord_neo = []
coord_meoh= []
force = []
...
r_th = 7.0  # editable
w_th = 0.8  # editable
box_size = 43.11145  # from NVT simulation
n_traj = 20  # Number of repetitive steps to simulate the "mixed state"
# Rand function
...
for line in f:
    ...
    if(k == 1 and len(line_element) == 1):
        ...
        force = []
        line_neopentane = ""
    ''' This part is not needed in the post-processed cg trj -> it's already in str_b
    ...
    if(k == 0 and line_element[0] != 'ITEM:'):  # Scan for all
        ...
        if ( len(index_temp) > 1000 ):
            ...
            # Final type for the molecule is changed!
            # TYPE 1: (methanol)   -> less dense MeOH
            # TYPE 2: (neopentane) -> more dense MeOH
            # TYPE 3: (N/A)        -> neopentane
            line_neopentane += line_element[0] + " 3"  # type 3
            for iter_line in range(2,8):
