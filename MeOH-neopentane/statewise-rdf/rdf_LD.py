        ...
        if (time_element < 1000000):
            ...
            if(time_element > -1):
                if(header_line_counter == 0 and line_elements[0] != 'ITEM:' and int(line_elements[0]) <= 1000):
                    pos_x[n_particles_read] = (float(line_elements[2])* box)
                    pos_y[n_particles_read] = (float(line_elements[3])* box)
                    pos_z[n_particles_read] = (float(line_elements[4])* box)
            ...
            if(header_line_counter == 1 and len(line_elements) == 1):
                ...
                if (n_particles_read > 0):
                    ...
                    if (poi > 0.0):
                        ...
                        g_temp = (g_temp / num_ideal) / float(poj)
                        zero_flag = 0
                    else:
                        zero_flag = 1
